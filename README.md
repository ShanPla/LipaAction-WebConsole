...
- **Agency progress arrives within about a minute, not instantly.** The live channel
  carries changes to reports; an agency's acknowledgement or resolution is stored
  separately, so while the queue is on screen the console checks for it about once a
  minute and updates only when something changed.
